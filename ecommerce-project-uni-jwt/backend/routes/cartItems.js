...
router.get('/', async (req, res) => {
  ...
  let cartItems = await CartItem.findAll({ where });
...
router.post('/', async (req, res) => {
  ...
  if (cartItem) {
    ...
    cartItem = await CartItem.create({ productId, quantity, deliveryOptionId: "1", userId });
  }

  console.log(`CartItems: ${productId} added successfully`);
...
router.delete('/:productId', async (req, res) => {
  ...
  console.log(`CartItems: ${productId} deleted successfully`);
  res.status(204).send();
});
